...
upsert_job() {
  ...
  if gcloud scheduler jobs describe "$name" --location="$REGION" --project="$GCP_PROJECT_ID" --account="$ACCOUNT" >/dev/null 2>&1; then
    gcloud scheduler jobs update http "$name" \
      --location="$REGION" --project="$GCP_PROJECT_ID" --account="$ACCOUNT" \
      --clear-headers --quiet --format=none
    gcloud scheduler jobs update http "$name" \
      --location="$REGION" --project="$GCP_PROJECT_ID" --account="$ACCOUNT" \
      --schedule="$schedule" --time-zone="Asia/Taipei" \
      --uri="${RUN_URL}${path}" --http-method=POST \
      --update-headers="X-Cron-Secret=${CRON_SECRET},Content-Type=application/json" \
      --message-body='{}' --attempt-deadline="$deadline" \
      --max-retry-attempts=3 --quiet --format=none
    return
  fi
